...
const Latest = () => {
    const { data, isError, isLoading }: { data: Data[]; isError: boolean; isLoading: boolean; } = fetcher('api/posts')
    if (isLoading) return <LazyDisplay />
    if (isError) return <Error />
    return (
        <section className="container mx-auto md:px-20 pt-10 pb-0">
        ...
        <div className='item mx-6 pt-0 items-center text-center col-start justify-start justify-items-center' >
            ...
            <div className="info flex justify-center flex-col py-4">
                ...
                <p className='text-sm line-clamp-3 pb-2' >
                    {data.description.slice(0, 185).concat(`...`)}
                    <Link href={`/posts/${data.id}`}><a className='text-orange-400' >know more</a></Link>
                </p>
